...
async function callWithRetry(fn, retries = 2, delay = 1500) {
  ...
  try {
    ...
    if (isModelNotFound) {
      throw new Error(
        `Gemini model "${DEFAULT_MODEL}" is unavailable for this API key. Set VITE_GEMINI_MODEL to a supported model (example: gemini-1.5-flash) and restart the dev server.`
      );
    }
